...
# ------------------------------
# 9. SKJUL SIDSTE BRUGER VED LOGIN
# ------------------------------
...
# ------------------------------
# 10. EKSTRA ADMIN OG SERVEROPERATOR
# ------------------------------
...
foreach ($ou in $OUlist) {
    ...
    if ($superUser) {
        ...
        Write-Host "Delegerer rettigheder for $($superUser.SamAccountName) i OU $ou..."
        dsacls $ouDN /G "$userSam:RPWP" | Out-Null
        dsacls $ouDN /G "$userSam:CCDC" | Out-Null
    }
}
Write-Host "Opsætning færdig!" -ForegroundColor Green
